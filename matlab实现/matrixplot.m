...
%
%	matrixplot(data)	绘制矩阵色块图，	data 为实值矩阵，每一个元素对应一个色块，色
%	块颜色由元素值大小决定。
%
...
%	1 行 3 列的红、绿、蓝三元色灰度值向量（	[r,g,b]  ）
%	'Auto'	---	根据矩阵元素值自动确定文字颜色
%
%	'XVarNames' ---	设定 X 轴方向需要显示的变量名（默认为	X1,X2,... ），其参数值为：
%	字符串矩阵或字符串元胞数组，若为字符串矩阵，其行数应与	data 的列数相同
%	若为字符串元胞数组，其长度应与	data 的列数相同。
%

%	'YVarNames' ---	设定  Y 轴方向需要显示的变量名（默认为	Y1,Y2,... ），其参数值为：
%	字符串矩阵或字符串元胞数组，若为字符串矩阵，其行数应与	data 的行数相同
%	若为字符串元胞数组，其长度应与	data 的行数相同。
%
...
%	matrixplot(x,'FigShap','e','FigSize','Auto','ColorBar','on','XVarNames',XVarNames,'YVar Names',XVarNames);
...
if isempty(XVarNames)
    ...
    if (iscell(XVarNames) && (numel(XVarNames) ~= n)) || (~iscell(XVarNames) && (size(XVarNames,1) ~= n))
        error('X	轴方向变量名应为字符串矩阵或字符串元胞数组，其长度与输入矩阵的列数相同 ');
    end
end
...
if isempty(YVarNames)
    ...
    if (iscell(YVarNames) && (numel(YVarNames) ~= m)) || (~iscell(YVarNames) && (size(YVarNames,1) ~= m))
        error('Y	轴方向变量名应为字符串矩阵或字符串元胞数组，其长度与输入矩阵的行数相同 ');
    end
end
...
%	'Auto' ---	将 x 轴刻度标签旋转	90 度， y 轴刻度标签不作调整
%	'Tril' ---	将 x 轴刻度标签旋转	90 度，并依次缩进，	y 轴刻度标签不作调整
%	'Triu' ---	将 x 轴刻度标签旋转	90 度， y 轴刻度标签依次缩进
%	Example:
%	MyTickLabel(gca,'Tril');
%
%	CopyRight ： xiezhh （谢中华）  ,2013.1	编写
...
%	根据散点数据绘制	3 维色块图
%	MyPatch(x,y,z,FigShape,FigSize)	x,y,z 是实值数组，用来指定色块中心点三维
%	坐标。 FigShape  是字符串变量，用来指定色块形状。
%	FigSize 是字符串变量，用来指定色块大小。
%
...
% 圆 形
t = linspace(0,2*pi,30)'; m = numel(t);
...
Zp = repmat(z,[m,1]); patch(Xp,Yp,Zp,'FaceColor','flat','EdgeColor','flat');
end
...
if ~any(strncmpi(FigShape,{'Square','Circle','Ellipse','Hexagon','Dial'},1))
    error(' 形状参数只能为	Square, Circle, Ellipse, Hexagon, Dial	之一 ');
end
if ~any(strncmpi(FigSize,{'Full','Auto'},3))
    
    error(' 图形大小参数只能为	Full, Auto	之一 ');
end
if ~any(strncmpi(FigStyle,{'Auto','Tril','Triu'},4))
    error(' 图形样式参数只能为	Auto, Tril, Triu	之一 ');
end
if ~any(strncmpi(FillStyle,{'Fill','NoFill'},3))
    error(' 图形填充样式参数只能为	Fill, NoFill	之一 ');
end
if ~any(strncmpi(DisplayOpt,{'On','Off'},2))
    error(' 显示数值参数只能为	On， Off	之一 ');
end
if ~any(strncmpi(ColorBar,{'On','Off'},2))
    error(' 显示颜色条参数只能为	On， Off	之一 ');
end
if ~any(strncmpi(GridOpt,{'On','Off'},2))
    error(' 显示网格参数只能为	On， Off	之一 ');
end
end
